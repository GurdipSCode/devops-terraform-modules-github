...
output "ruleset_ids" {
  ...
  value = {
    for name, ruleset in github_organization_ruleset.rulesets :
    name => ruleset.id
  }
}

output "ruleset_node_ids" {
  description = "Map of ruleset names to their node IDs"
  value = {
    for name, ruleset in github_organization_ruleset.rulesets :
    name => ruleset.node_id
  }
}

output "ruleset_details" {
  description = "Complete ruleset details"
  value = {
    for name, ruleset in github_organization_ruleset.rulesets :
    name => {
      id          = ruleset.id
